  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setStats(statsData);
        const ticketsData = await fetchData("/tickets");
      ...
      >
        <div className="relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter leading-tight">
            Bonjour, <span className="text-primary italic">{user?.name || "Utilisateur"}</span>
          </h1>
          <p className="mt-4 text-zinc-400 font-medium max-w-xl text-lg leading-relaxed">
            Votre plateforme Renault Axis est prete. Vous avez <span className="text-white font-bold">{stats.find(s => s.name === 'Total Tickets')?.value || '...'} interventions</span> actives ce mois-ci.
          </p>
            ...
            >
              <Card className="group border-border/40 bg-card/40 backdrop-blur-md hover:bg-card hover:border-primary/30 transition-all duration-500 rounded-[2rem] shadow-sm hover:shadow-xl hover:shadow-primary/5 overflow-hidden border-2 border-transparent">
                ...
                <CardContent className="px-8 pb-8 pt-0">
                  ...
                  <div className={`flex items-center gap-1.5 mt-4 text-[13px] font-bold ${item.changeType === 'positive' ? 'text-emerald-500' : item.changeType === 'negative' ? 'text-rose-500' : 'text-muted-foreground'
                    }`}>
                    <div className={`flex items-center gap-0.5 rounded-full px-2 py-0.5 ${item.changeType === 'positive' ? 'bg-emerald-500/10' : item.changeType === 'negative' ? 'bg-rose-500/10' : 'bg-muted'
                      }`}>
        ...
        <Card className="lg:col-span-2 rounded-[2.5rem] border-border/40 shadow-sm overflow-hidden bg-card/40 backdrop-blur-md border-2 border-transparent hover:border-border/60 transition-all">
          <CardHeader className="flex flex-row items-center justify-between bg-muted/10 border-b border-border/40 p-8">
            <div>
              ...
              <CardDescription className="font-semibold mt-1">Dernieres demandes de service enregistrees.</CardDescription>
            </div>
          ...
          <CardContent className="p-0">
            {recentTickets.length === 0 ? (
              <div className="p-20 text-center flex flex-col items-center">
                ...
                <p className="text-muted-foreground font-bold italic">Aucune intervention recente.</p>
              </div>
            ) : (
              <Table>
                ...
                <TableBody>
                  {recentTickets.map((ticket) => (
                    <TableRow key={ticket.id} className="hover:bg-muted/30 border-border/40 transition-colors group">
                      ...
                      <TableCell>
                        <Badge className={`rounded-xl px-3 py-1 text-[11px] font-black uppercase tracking-wider shadow-sm border-none ${ticket.status === 'OPEN' ? 'bg-blue-500/10 text-blue-600' :
        ...
        <div className="space-y-8">
          <Card className="rounded-[2.5rem] border-border/40 bg-card/40 backdrop-blur-md shadow-sm overflow-hidden p-8 border-2 border-transparent">
            ...
            <CardContent className="p-0">
              <div className="space-y-6">
                {[
                  { label: "Backend API", status: "Operationnel", color: "bg-emerald-500" },
                  { label: "OCR Engine", status: "Optimal", color: "bg-emerald-500" },
                  { label: "PostgreSQL", status: "Connecte", color: "bg-emerald-500" },
          ...
          >
            <div className="relative z-10">
              ...
              <p className="text-sm font-semibold mt-3 opacity-90 leading-relaxed">Une question ? Une assistance technique ? Nos experts Renault Axis sont a votre ecoute.</p>
